...
export async function GET() {
  return csvSampleResponse("expenses_import_sample.csv",
    ["Invoice #", "Invoice Date", "Category", "Supplier Nick Name", "Address", "HST #", "Phone", "Email", "Subtotal", "HST Amount", "HST Exempted", "Amount", "Description", "Payment Date", "Payment Method", "Transaction Ref. No."],
    ["1", "2026-07-21", "Office", "Example Supplier", "Toronto", "", "416-555-0100", "supplier@example.com", "100", "13", "No", "113", "Payment", "2026-07-21", "EFT", "REF-001"]);
}
...
// Map normalized CSV header → Expenditure model field
...
export async function POST(request: NextRequest) {
  try {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (!rec.supplierNickName) {
        errors.push(`Row ${i + 1}: Missing required field 'supplierNickName'`);
        continue;
      }

      // Clean up empty paymentRow
      if (Object.keys(rec.paymentRows[0]).length === 0) {
        rec.paymentRows = [];
      }
    ...
    if (toInsert.length > 0) {
      const result = await Expenditure.insertMany(toInsert, { ordered: false });
      imported = result.length;
    }

    return NextResponse.json(
      {
        message: `Successfully imported ${imported} expenditure record${imported !== 1 ? "s" : ""}`,
        imported,
        errors,
    ...
    return NextResponse.json({ error: "Failed to import expenditure: " + error.message }, { status: 500 });
  }
}
