...
type SortField = "createdAt" | "name" | "phone" | "companyName" | "fax" | "email" | "address";
...
export default function SolicitorsPage() {
  ...
  const handleExport = () => {
    ...
    window.location.href = ids ? `/api/solicitors/export?ids=${encodeURIComponent(ids)}` : "/api/solicitors/export";
  };
  const toggleAll = () => setSelectedIds(
    paginatedSolicitors.length > 0 && paginatedSolicitors.every((solicitor) => selectedIds.has(solicitor._id))
      ? new Set()
      : new Set(paginatedSolicitors.map((solicitor) => solicitor._id))
  );
    ...
    <main className="flex-1 w-full px-4 sm:px-6 py-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <a href="/api/solicitors/import" download className="flex items-center gap-1.5 px-3 py-2 border border-blue-200 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded-lg text-sm font-medium transition-colors" title="Download import sample">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50/50">
                <th className="px-3 py-3 w-10"><input type="checkbox" checked={paginatedSolicitors.length > 0 && paginatedSolicitors.every((solicitor) => selectedIds.has(solicitor._id))} onChange={toggleAll} aria-label="Select all solicitors on this page" className="w-4 h-4 cursor-pointer" /></th>
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                ...
                <tr><td colSpan={8} className="px-3 py-12 text-center text-gray-400">No Data available</td></tr>
              ) : (
                paginatedSolicitors.map((s) => (
                  <tr key={s._id} className="hover:bg-gray-50/50 transition-colors cursor-pointer" onClick={() => openEditModal(s)}>
                    <td className="px-3 py-3" onClick={(event) => event.stopPropagation()}><input type="checkbox" checked={selectedIds.has(s._id)} onChange={() => setSelectedIds((previous) => { const next = new Set(previous); if (next.has(s._id)) next.delete(s._id); else next.add(s._id); return next; })} aria-label={`Select ${s.name}`} className="w-4 h-4 cursor-pointer" /></td>
