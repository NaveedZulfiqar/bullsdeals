...
type SortField = "invoiceNumber" | "invoiceDate" | "customer" | "category" | "amount";
...
export default function IncomePage() {
  ...
  const fetchCategories = useCallback(async () => {
    try {
      ...
      if (res.ok) {
        ...
        setCategories(all.filter((c) => c.type === "INCOME"));
      }
    } catch { }
  }, []);
  ...
  const fetchIncomes = useCallback(async () => {
    try {
      ...
      if (filterCustomer) p.set("filterCustomer", filterCustomer);
    ...
    filterCustomer, filterCategory, filterRefNo, filterMethod]);

  useEffect(() => {
    const t = setTimeout(fetchIncomes, 300);
  ...
  const handleDelete = async (id: string) => {
    ...
    if (res.ok) { setIncomes(p => p.filter(i => i._id !== id)); setDeleteConfirm(null); setTotal(t => t - 1); }
  };

  const openAdd = async () => {
    // Get next invoice number
    const res = await fetch("/api/income?pageSize=1&sortField=invoiceNumber&sortOrder=desc");
    let nextNum = 1;
    if (res.ok) {
      const data = await res.json();
      if (data.incomes?.length > 0) nextNum = (data.incomes[0].invoiceNumber || 0) + 1;
    }
    const f = emptyForm();
  ...
  const openEdit = (inc: Income) => {
    ...
    setForm({
      invoiceNumber: String(inc.invoiceNumber || ""),
      invoiceDate: inc.invoiceDate || "",
      category: inc.category || "",
      customer: inc.customer || "",
      customerNickName: inc.customerNickName || "",
      address: inc.address || "",
      hstNumber: inc.hstNumber || "",
      phone: inc.phone || "",
      email: inc.email || "",
      invoiceItems: inc.invoiceItems?.length ? inc.invoiceItems : [],
      subtotal: String(inc.subtotal ?? "0.00"),
      hstAmount: String(inc.hstAmount ?? "0.00"),
      hstExempted: inc.hstExempted || false,
      amount: String(inc.amount ?? "0.00"),
      paymentRows: inc.paymentRows?.length ? inc.paymentRows : [emptyPaymentRow()],
    });
    setFormErrors({});
    setShowModal(true);
  };
  ...
  // ── Form field handlers ───────────────────────────────────────────────────
  const setField = (name: keyof IncomeFormData, value: any) => {
  ...
  const removeItem = (idx: number) => {
    setForm(p => {
      ...
      const subtotal = items.reduce((s, it) => s + (it.amount || 0), 0);
      const hstAmount = p.hstExempted ? 0 : items.reduce((s, it) => s + (it.hstAmount || 0), 0);
      return { ...p, invoiceItems: items, subtotal: subtotal.toFixed(2), hstAmount: hstAmount.toFixed(2), amount: (subtotal + hstAmount).toFixed(2) };
    });
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.ok) { setShowModal(false); fetchIncomes(); }
      else { const d = await res.json(); setFormErrors({ form: d.error || "Failed to save" }); }
  ...
  const handleExport = () => {
    ...
    window.location.href = ids ? `/api/income/export?ids=${encodeURIComponent(ids)}` : "/api/income/export";
  };
  const toggleAll = () => setSelectedIds(
    incomes.length > 0 && incomes.every((income) => selectedIds.has(income._id))
      ? new Set()
      : new Set(incomes.map((income) => income._id))
  );
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    fd.append("file", file);
    try {
      const res = await fetch("/api/income/import", { method: "POST", body: fd });
      const data = await res.json();
      if (res.ok) {
  ...
  return (
    <main className="flex-1 w-full px-4 sm:px-6 py-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-[#1B2559]">Income</h1>
          <p className="mt-1 text-xs text-gray-500">Trade commissions, brokerage splits, referral fees, and generated monthly invoices sync automatically.</p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          {/* icon-style export/import like screenshot toolbar */}
          <button onClick={() => { }} title="Print" className="p-2 border border-gray-200 rounded-lg hover:bg-gray-50 text-gray-500 cursor-pointer">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z" /></svg>
          </button>
          <button onClick={handleExport} title={selectedIds.size ? `Export ${selectedIds.size} selected records` : "Export all CSV records"} className="p-2 border border-gray-200 rounded-lg hover:bg-gray-50 text-red-500 cursor-pointer">
            <Download className="w-4 h-4" />
          </button>
          <a href="/api/income/import" download title="Download import sample" className="p-2 border border-gray-200 rounded-lg hover:bg-blue-50 text-blue-600 cursor-pointer">
            <Download className="w-4 h-4" />
          </a>
          <button onClick={() => fileInputRef.current?.click()} title="Import CSV" className="p-2 border border-gray-200 rounded-lg hover:bg-gray-50 text-green-600 cursor-pointer">
            <Upload className="w-4 h-4" />
          </button>
          <button onClick={() => { }} title="Export Excel" className="p-2 border border-gray-200 rounded-lg hover:bg-gray-50 text-green-700 cursor-pointer">
          ...
          <button onClick={openAdd}
            className="flex items-center gap-1.5 px-4 py-2 bg-[#1B2559] hover:bg-[#151d47] text-white rounded-lg text-sm font-semibold transition-colors cursor-pointer whitespace-nowrap">
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-xl p-6 max-w-sm w-full mx-4">
            <h3 className="text-lg font-bold text-[#2C2C2C] mb-2">Delete Income Record</h3>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50/50">
                <th className="px-3 py-3 w-10"><input type="checkbox" checked={incomes.length > 0 && incomes.every((income) => selectedIds.has(income._id))} onChange={toggleAll} aria-label="Select all income records on this page" className="w-4 h-4 cursor-pointer" /></th>
                <SH field="invoiceDate" label="Dated" />
                <SH field="invoiceNumber" label="Invoice No" />
                <SH field="customer" label="Head Name" />
              ...
              <tr className="border-b border-gray-200 bg-white">
                ...
                <td className="px-3 py-2">{FI(filterCustomer, setFilterCustomer)}</td>
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                ...
                return (
                  <tr key={inc._id} className="hover:bg-gray-50/50 transition-colors">
                    <td className="px-3 py-3"><input type="checkbox" checked={selectedIds.has(inc._id)} onChange={() => setSelectedIds((previous) => { const next = new Set(previous); if (next.has(inc._id)) next.delete(inc._id); else next.add(inc._id); return next; })} aria-label={`Select invoice ${inc.invoiceNumber}`} className="w-4 h-4 cursor-pointer" /></td>
                    <td className="px-3 py-3 text-sm text-gray-700 whitespace-nowrap">{fmtDate(inc.invoiceDate)}</td>
                    <td className="px-3 py-3 text-sm font-medium text-[#1B2559]">{inc.invoiceNumber}</td>
                    <td className="px-3 py-3 text-sm text-gray-700 max-w-[140px] truncate" title={inc.customer}>{inc.customer || "-"}</td>
                    <td className="px-3 py-3 text-sm text-gray-700">
                      <span>{inc.category || "-"}</span>
                      {inc.isAutoGenerated && (
                        <span className="ml-2 inline-flex rounded-full bg-blue-50 px-2 py-0.5 text-[10px] font-semibold text-blue-700">
                          {inc.sourceType === "trade" ? `Auto · Trade #${inc.tradeNumber || ""}` : "Auto · Monthly"}
                        </span>
                      )}
                    </td>
                    <td className="px-3 py-3 text-sm font-medium text-gray-800 whitespace-nowrap">{fmtCurrency(inc.amount)}</td>
                    ...
                    <td className="px-3 py-3 text-sm text-gray-700 whitespace-nowrap">
                      {pr?.paymentDate ? fmtDate(pr.paymentDate) : "-"}
                    </td>
                    <td className="px-3 py-3">
                      {inc.isAutoGenerated ? (
                        <span className="text-[10px] font-medium text-blue-600" title="Update this record from its source">Synced</span>
                      ) : (
                        <div className="flex items-center gap-1.5">
                          ...
                          <button onClick={() => setDeleteConfirm(inc._id)}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-3xl max-h-[92vh] overflow-y-auto">
            ...
            <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-gray-100 sticky top-0 bg-white z-10 rounded-t-xl">
              <h2 className="text-lg font-bold text-[#2C2C2C]">{editingId ? "Edit Income" : "Add Income"}</h2>
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              {/* Row 2: Customer + Nick Name */}
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label req>Customer</Label>
                  <div className="flex gap-1.5">
                    <Field
                      value={form.customer}
                      onChange={e => setField("customer", e.target.value)}
                      placeholder="Customer"
                      cls={`flex-1 ${formErrors.customer ? "border-red-400" : ""}`}
                    />
                    <button
                ...
                <div>
                  ...
                  <Field
                    ...
                    onChange={e => setField("customerNickName", e.target.value)}
            ...
            <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-100 bg-gray-50/40 rounded-b-xl">
              ...
              <button
                onClick={handleSave}
                disabled={saving}
                className="px-6 py-2 text-sm font-semibold text-white bg-[#1B2559] hover:bg-[#151d47] disabled:opacity-60 rounded-lg cursor-pointer transition-colors"
              >
                {saving ? "Saving…" : "Save"}
              </button>
            </div>
