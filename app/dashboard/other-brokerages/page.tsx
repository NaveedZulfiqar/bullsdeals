...
type SortField = "createdAt" | "name" | "phone" | "email" | "street" | "city" | "province" | "postalCode";
...
export default function OtherBrokeragePage() {
  ...
  const handleExport = () => {
    ...
    window.location.href = ids ? `/api/other-brokerages/export?ids=${encodeURIComponent(ids)}` : "/api/other-brokerages/export";
  };
  const toggleAll = () => setSelectedIds(
    brokerages.length > 0 && brokerages.every((brokerage) => selectedIds.has(brokerage._id))
      ? new Set()
      : new Set(brokerages.map((brokerage) => brokerage._id))
  );
    ...
    <main className="flex-1 w-full px-4 sm:px-6 py-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          {/* Import CSV */}
          <a
            href="/api/other-brokerages/import"
            download
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50/50">
                <th className="px-3 py-3 w-10"><input type="checkbox" checked={brokerages.length > 0 && brokerages.every((brokerage) => selectedIds.has(brokerage._id))} onChange={toggleAll} aria-label="Select all brokerages" className="w-4 h-4 cursor-pointer" /></th>
                ...
                brokerages.map((b) => (
                  ...
                  >
                    <td className="px-3 py-3" onClick={(event) => event.stopPropagation()}>
                      <input type="checkbox" checked={selectedIds.has(b._id)} onChange={() => setSelectedIds((previous) => { const next = new Set(previous); if (next.has(b._id)) next.delete(b._id); else next.add(b._id); return next; })} aria-label={`Select ${b.name}`} className="w-4 h-4 cursor-pointer" />
                    </td>
