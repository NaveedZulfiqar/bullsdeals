...
const payrollDropdownItems = [
  { name: "Employees", href: "/dashboard/employees" },
  { name: "Generate Payroll", href: "/dashboard/payroll/generate" },
  { name: "Payroll Settings", href: "/dashboard/payroll/settings" },
];


const accountingDropdownItems = [
  { name: "Income", href: "/dashboard/accounting/income" },
  { name: "Expenditure", href: "/dashboard/accounting/expense" },
  { name: "Reconciliation", href: "/dashboard/accounting/reconciliation" },
  { name: "Ledger", href: "/dashboard/accounting/ledger" },
  { name: "Monthly Collection", href: "/dashboard/accounting/monthly-collection" },
...
export default function Navbar() {
  ...
  const getActiveTab = () => {
    ...
    if (pathname.startsWith("/dashboard/accounting") || pathname.startsWith("/dashboard/reports") || pathname.startsWith("/dashboard/employees") || pathname.startsWith("/dashboard/payroll")) return "Accounting";
    ...
    <>
      <nav className="sticky top-0 z-50 bg-white border-b border-gray-200 shadow-sm h-16 flex items-center justify-between px-4 sm:px-6 w-full">
        ...
        <div className="hidden xl:flex items-center gap-0.5 flex-1 justify-center px-4" ref={dropdownRef}>
          {navItems.map((item) => (
            <div key={item.name} className="relative">
              <button
                onClick={() => {
                  if (item.name === "Masters") {
                    ...
                    setOpenDropdown(openDropdown === "Accounting" ? null : "Accounting");
                ...
                <div className="absolute left-0 top-full mt-1 w-52 bg-white border border-gray-200 rounded-lg shadow-lg py-1.5 z-50">
                  {mastersDropdownItems.map((dropItem) => (
                    <button
                      ...
                      className={`w-full text-left px-4 py-2.5 text-sm font-medium transition-colors cursor-pointer ${pathname.startsWith(dropItem.href)
              ...
              {/* Accounting Dropdown */}
              {item.name === "Accounting" && openDropdown === "Accounting" && (
                <div className="absolute left-0 top-full mt-1 w-60 bg-white border border-gray-200 rounded-lg shadow-lg py-1.5 z-50">
                  {accountingDropdownItems.map((dropItem) => (
                    <button
                      key={dropItem.name}
                      onClick={() => {
                        if (dropItem.href !== "#") {
                      ...
                      className={`w-full text-left px-4 py-2.5 text-sm font-medium transition-colors cursor-pointer ${dropItem.href !== "#" && pathname.startsWith(dropItem.href)
                        ? "text-[#FD7E14] bg-orange-50 border-l-3 border-[#FD7E14]"
                        : "text-gray-700 hover:text-[#FD7E14] hover:bg-orange-50"
                        }`}
                    >
                  ...
                  <div className="mx-3 my-1 border-t border-gray-100" />
                  <div className="flex items-center gap-2 px-4 pb-1 pt-2 text-[10px] font-bold uppercase tracking-[0.14em] text-gray-400">
                    <Banknote className="h-3.5 w-3.5" />
                    Payroll
                  </div>
                  {payrollDropdownItems.map((dropItem) => (
                    <button
                      key={dropItem.name}
                      onClick={() => {
                        router.push(dropItem.href);
                      ...
                      className={`w-full border-l-2 border-transparent py-2 pl-9 pr-4 text-left text-sm font-medium transition-colors cursor-pointer ${pathname.startsWith(dropItem.href)
                        ? "text-[#FD7E14] bg-orange-50 border-l-3 border-[#FD7E14]"
                        : "text-gray-700 hover:text-[#FD7E14] hover:bg-orange-50"
                        }`}
                    >
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          {/* Settings Button */}
          <button aria-label="Settings" className="hidden sm:flex w-9 h-9 rounded-xl bg-white border border-gray-200 items-center justify-center text-gray-500 hover:text-[#FD7E14] hover:border-[#FD7E14] transition-all shadow-sm cursor-pointer">
          ...
          <div className="absolute top-0 left-0 w-72 max-w-full h-full bg-white shadow-xl flex flex-col py-4">
            {navItems.map((item) => (
              <div key={item.name}>
                <button
                  onClick={() => {
                    if (item.name === "Masters") {
                      ...
                      setOpenDropdown(openDropdown === "Accounting-mobile" ? null : "Accounting-mobile");
                    } else if (!item.hasDropdown && item.href !== "#") {
                      router.push(item.href);
                      setIsMobileMenuOpen(false);
                    }
                  }}
                  className={`flex items-center gap-3 px-5 py-3 text-sm font-medium transition-colors cursor-pointer w-full ${activeTab === item.name
                ...
                {item.name === "Accounting" && openDropdown === "Accounting-mobile" && (
                  <div className="bg-gray-50 pl-12">
                    {accountingDropdownItems.map((dropItem) => (
                      <button
                        key={dropItem.name}
                    ...
                    <div className="flex items-center gap-2 border-t border-gray-200 px-4 pb-1 pt-3 text-[10px] font-bold uppercase tracking-[0.14em] text-gray-400">
                      <Banknote className="h-3.5 w-3.5" />
                      Payroll
                    </div>
                    {payrollDropdownItems.map((dropItem) => (
